...
import { DebitNote, ScreenPermission, AppUser } from '../types';
...
const DebitNotesView = () => {
    ...
    const [selectedCOGS, setSelectedCOGS] = useState<DebitNote | null>(null);
    const [creditNotes, setDebitNotes] = useState<DebitNote[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                console.log('FETCHED CREDIT NOTES:', data); setDebitNotes(data);
            } catch (err) {
    ...
    useEffect(() => {
        setPerms({ screenId: 'debit-notes', view: true, add: true, edit: true, delete: true });
    }, [currentUser]);
    ...
    const copyToClipboard = (data: DebitNote[]) => {
        const header = "Issue Date\tReference\tTarget Entity\tDescription\tAmount\tStatus\tTimestamp";
        const rows = data.map(q =>
            `${q.issueDate}\t${q.reference}\t${q.supplier?.name || q.customer?.name || ''}\t${q.description || ''}\t${q.amount}\t${q.status}\t${q.timestamp || ''}`
        ).join('\n');
    ...
    const filteredData = useMemo(() => {
        ...
        if (customerName) {
            result = result.filter(q => (q.customer?.name || q.supplier?.name || '').toLowerCase() === customerName.toLowerCase());
        }
        if (statusFilter !== 'All') {
        ...
        return result.sort((a, b) => {
            let valA: any = a[sortColumn as keyof DebitNote] || '';
            let valB: any = b[sortColumn as keyof DebitNote] || '';
    ...
    }, [creditNotes, searchQuery, refreshTrigger, customerName, statusFilter, sortColumn, sortDirection]);
    ...
    const allColumns = [
        ...
        },
        {
            ...
            accessor: (o: any) => (
                <div className="flex items-center gap-2">
                    <button
                        onClick={() => navigate(`/debit-notes/view/${o.id}`)}
                    ...
                    {perms?.edit !== false && (
                        <button
                            onClick={() => navigate(`/debit-notes/edit/${o.id}`)}
        ...
        {
            id: 'Entity',
            header: <div className="flex items-center cursor-pointer group hover:text-indigo-600 transition-colors" onClick={() => handleSort('Entity')}>Target Entity <SortIcon column="Entity" /></div>,
            className: 'min-w-[200px]',
            accessor: (o: any) => (
                <div>
                    <span className="font-bold text-slate-700">{o.supplier?.name || o.customer?.name || '—'}</span>
                    <span className="block text-[10px] text-slate-400 uppercase tracking-widest mt-0.5">{o.supplierId ? 'Supplier' : 'Customer'}</span>
                </div>
            )
        },
        {
            ...
            className: 'whitespace-nowrap',
            accessor: (o: any) => (
                <span className="font-medium text-slate-500 line-clamp-1 max-w-[200px]">
                    {o.description || '—'}
                </span>
            )
        },
        ...
        <div className="p-8 space-y-8 max-w-[1600px] mx-auto animate-in fade-in duration-700 font-sans">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Debit Notes</h1>
            ...
            {isSelectionMode && (
                <div className="bg-indigo-600 px-8 py-4 rounded-[24px] flex items-center justify-between shadow-xl shadow-indigo-200/50 animate-in slide-in-from-top-4 duration-500 border border-white/10 backdrop-blur-md mb-8 max-w-[1200px]">
                    <div className="flex items-center space-x-8">
                        ...
                        <div className="flex items-center space-x-4">
                            <button
                                onClick={() => navigate(`/debit-notes/print-batch?ids=${selectedIds.join(',')}`)}
                    ...
                    emptyMessage={
                        isLoading ? (
                            <div className="flex flex-col items-center justify-center space-y-4 py-20">
                                ...
                                <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Loading debit notes...</p>
                            </div>
                    ...
                    <div className="relative" ref={batchOpsRef}>
                        ...
                        {isBatchOpsOpen && (
                            <div className="absolute bottom-full right-0 mb-2 w-56 bg-white border border-gray-200 shadow-xl rounded-md py-1 z-[100] animate-in slide-in-from-bottom-2 duration-300 overflow-hidden text-left">
                                <button
                                    onClick={() => { navigate('/debit-notes/edit-columns'); setIsBatchOpsOpen(false); }}
