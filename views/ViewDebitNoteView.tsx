...
import { DebitNote, Customer } from '../types';
import {
    Printer,
    FileText,
    Mail,
...
const ViewDebitNoteView = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            setIsLoading(true);
            try {
                const [cns, custs, sups, codes] = await Promise.all([
                    apiService.getDebitNotes(),
                    apiService.getCustomers(),
                    (apiService as any).getSuppliers ? (apiService as any).getSuppliers() : Promise.resolve([]),
                    apiService.getTaxCodes().catch(() => [])
                ]);
                setDebitNotes(cns);
                setTaxCodes(codes);
                const cn = cns.find((c: any) => c.id === id || c.id === `dn-${id}` || c.id.toString() === id);
                if (cn) {
                    setDebitNote(cn);
                    if (cn.supplier) {
                        const sup = sups.find((s: any) => s.id === cn.supplier?.id || s.name === cn.supplier?.name);
                        setEntityData(sup);
                        setEntityType('Supplier');
                    } else if (cn.customer) {
                        const cust = custs.find((c: any) => c.id === cn.customer?.id || c.name === cn.customer?.name);
                        setEntityData(cust);
    ...
    const cnIndex = debitNotes.findIndex(c => c.id === id || c.id === `dn-${id}` || c.id.toString() === id);

    const entityEmail = entityData?.email || (debitNote ? `${(debitNote.customer?.name || debitNote.supplier?.name || '').toLowerCase().replace(/\s+/g, '.')}@example.com` : '');

    const totals = useMemo(() => {
        if (!debitNote) return { subtotal: 0, tax: 0, total: 0, costOfSales: 0 };
        const options = debitNote.docOptions || {};
        const isTaxInclusive = options.amountsAreTaxInclusive || false;
        
        let subtotal = 0;
        let tax = 0;
        (debitNote.items || []).forEach((item: any) => {
        ...
        return { subtotal, tax, total: parseFloat(debitNote.amount) || 0, costOfSales: parseFloat(debitNote.costOfSales) || 0 };
    }, [debitNote, taxCodes]);
    ...
    if (!debitNote) return <div className="p-8 text-center text-slate-500 font-black uppercase tracking-widest">Debit Note not found.</div>;
        ...
        <div className="min-h-screen bg-[#f3f4f6]/50 flex flex-col font-sans">
            ...
            <div className="bg-[#f8fafc] border-b border-gray-300 px-6 py-3 flex items-center justify-between sticky top-0 z-50 no-print">
                <div className="flex items-center space-x-3">
                    ...
                    <button
                        onClick={() => navigate(`/debit-notes/edit/${debitNote.id}`)}
                        className="bg-white border border-gray-300 px-4 py-1.5 text-[12px] font-bold text-gray-700 rounded shadow-sm hover:bg-gray-50 flex items-center gap-2"
                    >
                        ...
                        <button
                            onClick={async () => {
                                ...
                                try {
                                    ...
                                    pdf.save(`${debitNote.reference || 'DebitNote'}.pdf`);
                                } catch (err: any) {
                ...
                <div className="flex items-center space-x-2">
                    <div className="flex bg-white border border-gray-300 rounded shadow-sm">
                        <button
                            onClick={() => navigate(`/debit-notes/view/${debitNotes[0].id}`)}
                            disabled={cnIndex <= 0}
                        ...
                        <button
                            onClick={() => navigate(`/debit-notes/view/${debitNotes[Math.max(0, cnIndex - 1)].id}`)}
                            disabled={cnIndex <= 0}
                    ...
                    <span className="text-[11px] font-bold text-gray-400 mx-2 uppercase tracking-widest">{cnIndex + 1} / {debitNotes.length}</span>
                    <div className="flex bg-white border border-gray-300 rounded shadow-sm">
                        <button
                            onClick={() => navigate(`/debit-notes/view/${debitNotes[Math.min(debitNotes.length - 1, cnIndex + 1)].id}`)}
                            disabled={cnIndex === debitNotes.length - 1}
                            className="px-3 py-1.5 text-gray-600 hover:bg-gray-50 border-r border-gray-300 disabled:opacity-30 flex items-center"
                        >
                            <ChevronRight size={14} />
                        </button>
                        <button
                            onClick={() => navigate(`/debit-notes/view/${debitNotes[debitNotes.length - 1].id}`)}
                            disabled={cnIndex === debitNotes.length - 1}
                            className="px-3 py-1.5 text-gray-600 hover:bg-gray-50 disabled:opacity-30 flex items-center"
                        >
                        ...
                        <div className="flex-1">
                            ...
                            <div className="mb-6">
                                <div className="flex justify-between items-start mb-1">
                                    <h1 className="text-xl font-bold text-slate-900 tracking-tight uppercase leading-none">Debit Note</h1>
                                </div>
                                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.2em]">Reference: {debitNote.reference}</p>
                            </div>

                            <div className="grid grid-cols-2 gap-12 items-start">
                                {/* Customer Info */}
                                <div>
                                    <h3 className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-4 border-b border-gray-50 pb-2">{entityType || 'Entity'}</h3>
                                    <p className="text-sm font-bold text-slate-900 uppercase tracking-tight mb-2">{debitNote.supplier?.name || debitNote.customer?.name || debitNote.supplierId || debitNote.customerId || '-'}</p>
                                    <div className="text-gray-500 space-y-1">
                                        <p className="whitespace-pre-wrap">{entityData?.billingAddress || entityData?.address || '-'}</p>
                                        {entityEmail && <p className="text-blue-600 lowercase">{entityEmail}</p>}
                                    </div>
                                </div>
                                ...
                                <div className="border-l border-gray-100 pl-12">
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex">
                                            ...
                                            <span className="font-semibold">{debitNote.issueDate}</span>
                                        </div>
                                        <div className="flex">
                                            <span className="w-32 text-gray-500">Sales Invoice:</span>
                                            <span className="font-semibold text-indigo-600">{debitNote.salesInvoice || '-'}</span>
                                        </div>
                                        <div className="flex">
                                            <span className="w-32 text-gray-500">Currency:</span>
                                            <span className="font-semibold">{debitNote.currency || 'ZMW'}</span>
                                        </div>
                                        {debitNote.timestamp && (
                                            <div className="flex">
                                                <span className="w-32 text-gray-500">Timestamp:</span>
                                                <span className="font-semibold text-[11px] text-slate-400 tracking-tight">{debitNote.timestamp}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                {(debitNote.items || []).map((item: any, idx: number) => (
                                    <tr key={idx}>
                                        ...
                                        <td className="px-4 py-4">
                                            <p className="font-semibold text-slate-900">{item.item?.itemCode || item.item?.itemName || (typeof item.item === 'string' ? item.item : '') || (item.itemId ? '-' : item.description) || '-'}</p>
                                        </td>
                                        <td className="px-4 py-4">
                    ...
                    <div className="flex justify-end">
                        <div className="w-80 space-y-3">
                            ...
                            <div className="flex justify-between items-center bg-slate-50 p-4 border-t-2 border-slate-900 mt-2 print-bg-slate-50">
                                <span className="text-[12px] font-bold uppercase tracking-[0.2em] text-slate-900">Debit Total</span>
                                <div className="text-right">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{debitNote.currency}</p>
                                    <p className="text-2xl font-bold text-slate-900 tracking-tighter">{debitNote.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                                </div>
                            </div>
                            <div className="flex justify-between items-center pt-2 px-4">
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Cost of Sales</span>
                                <span className="text-sm font-bold text-slate-600">{(debitNote.costOfSales || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                            </div>
                        </div>
                    </div>
                </div>
...
export default ViewDebitNoteView;
